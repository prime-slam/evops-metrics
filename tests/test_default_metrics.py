...
def test_half_fScore_iou_statistics_result():
    pred_labels = np.array([1, 1, 2, 3])
    gt_labels = np.array([4, 5, 6, 7])

    assert 0.5 == pytest.approx(fScore(pred_labels, gt_labels, "iou"))


def test_precision_real_data_iou_statistics():
    evops.metrics.constants.IOU_THRESHOLD = 0.1
    pred_labels = np.load("tests/data/pred_0.npy")
    gt_labels = np.load("tests/data/gt_0.npy")

    assert 0.0000201 == pytest.approx(
        float("{:.7f}".format(precision(pred_labels, gt_labels, "iou"))), 0.00001
    )
...
def test_recall_real_data_iou_statistics():
    ...
    assert 0.0000147 == pytest.approx(
        float("{:.7f}".format(recall(pred_labels, gt_labels, "iou"))), 0.00001
    )
...
def test_fScore_real_data_iou_statistics():
    ...
    assert 1.7e-05 == pytest.approx(
        float("{:.7f}".format(fScore(pred_labels, gt_labels, "iou"))), 0.00001
    )
